...
class Catchphrases: NSObject {
    ...
    class func nextRandomPhrase() -> String {
        let ramdomIndex = self.listOfPhrases.count.random()
        print("index = \(ramdomIndex), count = \(self.listOfPhrases.count)")
        return self.listOfPhrases[ramdomIndex]
    }
}
extension Int {
    func random() -> Int {
